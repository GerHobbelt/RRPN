...
def get_rroidb(imdb_name="icdar_2015_train"):	

	L_DATASET = []
    
	if imdb_name == "icdar_2015_train":
		L_ICDAR2015_RRC_PICK_TRAIN = get_ICDAR2015_RRC_PICK_TRAIN('train')
		print "L_ICDAR2015_RRC_PICK_TRAIN:", len(L_ICDAR2015_RRC_PICK_TRAIN)
	
        # dataset extend
    
		L_DATASET.extend(L_ICDAR2015_RRC_PICK_TRAIN)
		print "total", len(L_DATASET)
	elif imdb_name == "icdar_2015_test":
		L_ICDAR2015_TEST = get_ICDAR2015_test('test')
		print "L_ICDAR2015_TEST:", len(L_ICDAR2015_TEST)
        
		# dataset extend
        
 		L_DATASET.extend(L_ICDAR2015_TEST)
		print "total", len(L_DATASET)       
...
def get_ICDAR2015_test(mode):
	dir_path = "/workspace/mnt/group/fd-students/qiutairu/code/RRPN_ROOT/data/dataset/ICDAR_2015/ch4_test_images/"
	img_file_type = "jpg"

	gt_dir = "/workspace/mnt/group/fd-students/qiutairu/code/RRPN_ROOT/data/dataset/ICDAR_2015/Challenge4_Test_Task1_GT/"
...
# 			else:
			...
			if len(gt_ind) > 3:
				...
				boxes.append([x_ctr, y_ctr, height, width, angle])
				ori_boxes.append([int(ordi) for ordi in gt_ind[:8]])
			
		len_of_bboxes = len(boxes)
		gt_boxes = np.zeros((len_of_bboxes, 5), dtype=np.int16)	
		ori_gt_boxes = np.zeros((len_of_bboxes, 8), dtype=np.int16)	
		gt_classes = np.zeros((len_of_bboxes), dtype=np.int32)
		overlaps = np.zeros((len_of_bboxes, 2), dtype=np.float32) #text or non-text
		seg_areas = np.zeros((len_of_bboxes), dtype=np.float32)
		...
		for idx in range(len(boxes)):
			gt_boxes[idx, :] = [boxes[idx][0], boxes[idx][1], boxes[idx][2], boxes[idx][3], boxes[idx][4]]
			ori_gt_boxes[idx, :] = [ordi for ordi in ori_boxes[idx]]				
			gt_classes[idx] = 1 # cls_text
			overlaps[idx, 1] = 1.0 # cls_text
			seg_areas[idx] = (boxes[idx][2]) * (boxes[idx][3])
...
def get_ICDAR2015_RRC_TRAIN(mode):
	dir_path = "/workspace/mnt/group/fd-students/qiutairu/code/RRPN_ROOT/data/dataset/ICDAR_2015/ch4_training_images/"
	img_file_type = "jpg"

	gt_dir = "/workspace/mnt/group/fd-students/qiutairu/code/RRPN_ROOT/data/dataset/ICDAR_2015/ch4_training_localization_transcription_gt/"
	...
	for file_name in file_list:
		...
		if split[len(split) - 1] == img_file_type:
			img_list.append(file_name)
	print 'img_list.len(): {}'.format(len(img_list))
	

	for img_ind in range(len(img_list)):
		split = img_list[img_ind].split(".")
# 		print 'split in img_list: {}'.format(split)
		gt_list.append(cls + "_" + split[0] + ".txt")
...
def get_ICDAR2015_RRC_PICK_TRAIN(mode):
	...
	for idx in range(len(img_list)):
		...
		gt_split = []
		#print gt_name
		with codecs.open(gt_name, "r", "utf-8-sig") as gt_obj:
# 		gt_obj = open(gt_name, 'r')
...
# 		#print '-------------'
		for gt_line in gt_split:
			...
			if len(gt_ind) > 3 and gt_ind[8] != '###\r':
				...
				angle = 0
                # transfer(x1,y1,x2,y2,x3,y3,x4,y4) to (x,y,w,h,angle)
                # ensure long side and short side as width and height
				if edge1 > edge2:
				
					width = edge1
					height = edge2
                    # calc angle by using horizon edge
					if pt1[0] - pt2[0] != 0:
						angle = -np.arctan(float(pt1[1] - pt2[1]) / float(pt1[0] - pt2[0])) / 3.1415926 * 180
					else:
						angle = 90.0
				...
				easy_boxes.append([x_ctr, y_ctr, height, width, angle])
            # remove 80% of the unreadable text instance in training set
			if len(gt_ind) > 3 and gt_ind[8] == '###\r':
				#condinate_list = gt_ind[2].split(',')
